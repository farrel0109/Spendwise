    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between pt-4">
        <div>
          ...
          <p className="text-muted mt-1 ml-14">Save for your dreams</p>
        </div>
        <button
      ...
      {goals.length === 0 ? (
        <div className="bg-[var(--color-surface-elevated)] rounded-3xl p-16 text-center border border-dashed border-zinc-300 dark:border-white/5">
          ...
          <h3 className="text-xl font-bold text-primary mb-2">No goals yet</h3>
          <p className="text-muted max-w-sm mx-auto">Set a financial goal to stay motivated.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {goals.map((goal) => {
            ...
            return (
              <div key={goal.id} className="bg-[var(--color-surface-elevated)] p-6 rounded-3xl border border-zinc-200 dark:border-white/5 hover:border-[var(--accent-color)]/50 transition-all group hover:shadow-xl hover:shadow-[var(--accent-glow)]/5 hover:-translate-y-1 flex flex-col relative overflow-hidden">
                ...
                <div className="flex-1">
                  <h3 className="font-bold text-primary text-xl mb-1">{goal.name}</h3>
                ...
                <button
                  ...
                  className="mt-6 w-full py-3 bg-zinc-100 dark:bg-white/5 hover:bg-zinc-200 dark:hover:bg-[#2d3b4b] text-primary rounded-xl font-semibold border border-zinc-200 dark:border-white/5 transition-all flex items-center justify-center gap-2"
                >
                ...
                {goal.is_completed && (
                  <div className="absolute inset-0 bg-[var(--color-surface)]/80 backdrop-blur-sm flex flex-col items-center justify-center text-center p-6 animate-fadeIn">
                    ...
                    <h3 className="text-2xl font-bold text-primary mb-2">Goal Reached!</h3>
                    <p className="text-secondary">Congratulations on saving Rp {formatCurrency(goal.target_amount)}</p>
                  </div>
                )}
              </div>
            );
        ...
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-[var(--color-surface-elevated)] rounded-3xl overflow-hidden animate-slideUp shadow-2xl ring-1 ring-white/10">
            <div className="flex items-center justify-between px-6 py-5 border-b border-zinc-200 dark:border-white/5 bg-[var(--color-surface-elevated)]/50 backdrop-blur-md">
              ...
              <span className="text-lg font-bold text-primary">New Goal</span>
              <div className="w-10" />
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <div>
                <label className="block text-xs text-secondary mb-2 uppercase tracking-wider font-bold">Goal Name</label>
                <input
                  type="text"
                  value={formData.name}
              ...
              <div>
                <label className="block text-xs text-secondary mb-2 uppercase tracking-wider font-bold">Target Amount</label>
              ...
              <div>
                <label className="block text-xs text-secondary mb-3 uppercase tracking-wider font-bold">Icon & Color</label>
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center justify-center gap-2 p-3 rounded-xl border transition-all ${
                        formData.icon === item.name 
                          ...
                          : "bg-[var(--color-surface)] border-transparent text-muted hover:bg-zinc-100 dark:hover:bg-white/5"
                      }`}
                    >
      ...
      {showContribute && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-[var(--color-surface-elevated)] rounded-3xl overflow-hidden animate-slideUp shadow-2xl ring-1 ring-white/10 p-6">
            <h3 className="text-lg font-bold text-primary mb-4">Add Savings</h3>
            <form onSubmit={handleContribute}>
              <div className="flex items-center bg-[var(--color-surface)] rounded-xl px-4 border border-zinc-200 dark:border-white/5 focus-within:border-[var(--accent-color)] focus-within:ring-4 focus-within:ring-[var(--accent-color)]/10 transition-all mb-4">
                <span className="text-muted mr-2 font-medium">Rp</span>
                <input
                  type="number"
  ...
  return (
    <div className="premium-card p-6 rounded-3xl animate-pulse">
      <div className="flex justify-between mb-4">
        <div className="w-16 h-16 rounded-full bg-zinc-200 dark:bg-white/10" />
        <div className="w-8 h-8 bg-zinc-100 dark:bg-white/5 rounded-xl" />
      </div>
      <div className="h-5 w-32 bg-zinc-200 dark:bg-white/10 rounded mb-2" />
      <div className="h-3 w-20 bg-zinc-100 dark:bg-white/5 rounded mb-4" />
      <div className="space-y-2">
